#!/bin/bash
# https://github.com/Namia-M/OpenWrt-X
# common Module by Namia-M
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  sudo rm -rf build/common && git clone -b main --depth 1 https://github.com/Namia-M/common build/common
...
git clone -b main --depth 1 https://github.com/Namia-M/OpenWrt-X shangyou
...
if [[ "${GIT_REPOSITORY}" =~ (Namia-M/OpenWrt-X|Namia-M/autobuild) ]]; then
  rm -rf backups
...
curl -fsSL https://raw.githubusercontent.com/Namia-M/common/main/common.sh -o common.sh
if [[ $? -ne 0 ]]; then
  wget -q https://raw.githubusercontent.com/Namia-M/common/main/common.sh -O common.sh
fi
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  export GIT_REPOSITORY="Namia-M/OpenWrt-X"
  sudo rm -rf build common.sh

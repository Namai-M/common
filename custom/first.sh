...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  sudo rm -rf build/common && git clone -b main --depth 1 https://github.com/Namia-z/common build/common
...
git clone -b main --depth 1 https://github.com/Namia-z/OpenWrt-X shangyou
...
if [[ "${GIT_REPOSITORY}" =~ (Namia-z/OpenWrt-X|Namia-z/autobuild) ]]; then
  rm -rf backups
...
curl -fsSL https://raw.githubusercontent.com/Namia-z/common/main/common.sh -o common.sh
if [[ $? -ne 0 ]]; then
  wget -q https://raw.githubusercontent.com/Namia-z/common/main/common.sh -O common.sh
fi
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  export GIT_REPOSITORY="Namia-z/OpenWrt-X"
  sudo rm -rf build common.sh
